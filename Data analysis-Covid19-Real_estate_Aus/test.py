...
file_data_copy["date_sold"] = file_data_copy["date_sold"].str.replace(' 00:00:00', '')
file_data_copy["date_sold_year"] = [file_data_copy["date_sold"].str.split('-')[i][0] for i in
                                    range(0, len(file_data_copy["date_sold"]))]
file_data_copy["date_sold_month"] = [file_data_copy["date_sold"].str.split('-')[i][1] for i in
                                    range(0, len(file_data_copy["date_sold"]))]
file_data_copy["date_sold_day"] = [file_data_copy["date_sold"].str.split('-')[i][2] for i in
                                    range(0, len(file_data_copy["date_sold"]))]
print([file_data_copy["date_sold"].str.split('-')[i][0].astype(int) for i in
                                    range(0, len(file_data_copy["date_sold"]))])
#print(np.array(file_data_copy["date_sold"].str.split('-'))[:, 0])
